...
if($pre_date&&$end_date){
  $query = "SELECT * FROM board WHERE $category LIKE '%$search_title%' AND date BETWEEN '$pre_date' and '$end_date' ORDER BY $ord_ca DESC" ;
} else {
  $query = "SELECT * FROM board WHERE $category like '%$search_title%' order by $ord_ca desc";
}
...
?>
    ...
    <table class="table">
        ...
        if($pre_date&&$end_date){
          $query3 = "SELECT * FROM board WHERE $category LIKE '%$search_title%' AND date BETWEEN '$pre_date' AND '$end_date' order by $ord_ca DESC limit $start_num,$list";
        } else {
          $query3 = "SELECT * FROM board WHERE $category like '%$search_title%' order by $ord_ca desc limit $start_num,$list";
        }
